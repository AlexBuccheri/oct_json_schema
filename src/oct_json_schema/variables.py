...
def toHugo(raw_html):
    ...
    cleantext = cleantext.replace('</math>', '$')
    cleantext = cleantext.replace('<\\tt>', '</tt>')
    cleantext = cleantext.replace('"', '&quot;')
...
def format_description(desc, filter=lambda x: x, width=70, indent=''):
    # filter can be either of the function 'cleanhtml()' or 'leavehtml()'.
...
class Variables:
    ...
    def __init__(self, verbose=False, sources=None, tests=None, tutorials=None, manuals=None, json=None):
        """
        ...
        if tutorials:
            self.parse_tutorials(tutorials)

        if manuals:
            self.parse_manuals(manuals)
    ...
    def __getitem__(self, key):
        """
        Implement the [] operator for the variable class.
        """
    ...
    def parse_sources(self, srcdir):
        ...
        if not srcfiles:
            raise FileNotFoundError(f'No .F90 files found in {srcdir}')
        ...
        for f in srcfiles:
            ...
            with open(f, 'r') as source:
                ...
                line_number = 1
                while line != '':
                    if 'call parse_variable(' in line or 'parse_block(' in line and not '! ' in line:
            ...
            for key in keys_in_file:
                ...
                for call in parse_calls:
                    if "'" + key + "'" in call[0].lower() or '"' + key + '"' in call[0].lower():
    ...
    def parse_testfiles(self, testdir):
        ...
        for f in files:
            ...
            for line in input:
                words = line.split()
                if len(words) > 0:
                    key = words[0].strip().lstrip('%').lower()
                    if self.verbose:
                        print('  ' + key)
    ...
    def parse_tutorials(self, tutorialdir):

        files = glob.glob(tutorialdir + "/**/*.md", recursive=True)
        ...
        for f in files:
            ...
            input = open(f, 'r')
            for line in input:
                matches = re.findall('(\{\{< *variable "\w+")', line)
                for match in matches:
                    key = match.split()[-1].strip('"').lower()
                    if key in self.variables.keys():
                        tutorial = f.replace(tutorialdir, '')[1:-3]
                        if tutorial not in self.variables[key]['Tutorials']:
                            self.variables[key]['Tutorials'].append(tutorial)
            input.close()

    def parse_manuals(self, manualdir):

        files = glob.glob(manualdir + "/**/*.md", recursive=True)
        ...
        for f in files:
            ...
            for line in input:
                ...
                for match in matches:
                    key = match.split()[-1].strip('"').lower()
                    if key in self.variables.keys():
                        manual = f.replace(manualdir, '')[1:-3]
                        if manual not in self.variables[key]['Manuals']:
                            self.variables[key]['Manuals'].append(manual)
            input.close()
    ...
    def write_variable_md(self, key, file=sys.stdout, head=True):
        ...
        if (head):
            ...
            print('*Name* {{< variable "' + variable['Name'] + '" >}}<br>', file=file)
        section_name = variable['Section']
        section_string = '{{< versioned-link "variables/' + section_name.replace('::', '/').replace(' ',
                                                                                                    '_') + '"  "' + section_name + '" >}}'
        print('*Section* ' + section_string + '<br>', file=file)
        ...
        for option in variable['Options']:
            ...
            print('<li>   <b>' + option['Name'] + '</b>: </br>', file=file)
        ...
        for key in sorted(self.variables.keys()):
            ...
            for option in sorted(current['Options'], key=lambda opt: opt['Name']):
                if 'Value' in option:
                    ...
                    print('#define OPTION__' + key.upper() + '__' + option['Name'].upper() + ' (' + value + '_8)',
                          file=file)
    ...
    def check_untested(self, without_default=False, filename=None, whitelist=[]) -> []:
        """
        This function returns a list of variables are not covered by any test.
        """
        ...
        for key in self.variables.keys():
            current = self.variables[key]
            if (len(current['Testfiles']) == 0) and (current['Name'] not in whitelist):
                if without_default:
                    if not current['Default']:
                        untested.append(current['Name'])
                else:
                    untested.append(current['Name'])
    ...
    def check_undocumented(self, without_default=False, whitelist=[]) -> []:
        """
        This returns a list of variables are not covered by any tutorials or manual pages.
        """
        ...
        for key in self.variables.keys():
            current = self.variables[key]
            if (len(current['Tutorials']) + len(current['Manuals']) == 0) and (current['Name'] not in whitelist):
                if without_default:
                    if not current['Default']:
                        undocumented.append(current['Name'])
                else:
                    undocumented.append(current['Name'])
